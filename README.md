...
1. `\033`：八进制 **27**，对�?ASCII 码的 **“ESC�?*�?2. `\033[nF`：光标上�?n$行；
3. `\033[B`：光标下移一行；
4. `\033[nG`：光标移动到�?n$列；
5. `\033[2K`：删除本行；
6. `\033[2J`：清屏；
...
1. ### 基本公式
    ...
    1. 歌曲分数�?
        $Phigros$ �?*歌曲分数** ($score$) 由最�?$900000$ 分的**判定�?* ($accuracy\_score$) 与最�?$100000$ 分的**连击�?* ($combo\_score$�? 组成，即�?
        $$
    ...
    2. 判定分：
        ...
        1. 单个 $perfect$ 判定�?($perfect\_score$�? 的计算公式为�?
            $$
            perfect\_score = \frac{900000}{note} \tag{2}
            $$

        2. $good$ 判定�?($good\_score$�? �?perfect$判定分的 $65\%$，即�?
            $$
...
2. ### 算法原理
    ...
    �?(8)$中的 $note$ �?$score$ 为用户输入的参数，设 $score$ �?*目标分数** $goal$ �?\frac{5000}{note}$ �?*比例系数** $k$​，则有�?
    $$
    \begin{cases}
...
3. ### 算法设计
    ...
    根据 $Phigros$ 当前的游戏情况，对于�?(9)$中的各参数，我们可得出以下限制条件：
    ...
    根据�?(9)$和条�?(10)$，可设计算法如下�?
    1. 数据结构设计�?
        设计**输入检�?*�?*方案组合**�?*算法**�?*歌曲**五个参数结构，并使用动态数组存储方案。其中歌曲参数包括用户输入的 $note$�?goal$；算法参数包含了计算过程中用到的各个辅助参数；方案组合参数负责存储满足式$(9)$与条�?(10)$的参数组合；输入检查参数检查用户输入的数据是否合法。具体结构如下：
        ...
        1. 输入检查参数结构：
            ...
            typedef struct input_check {
                int min;
                int max;
                const char* name; ///< 当前输入数据的名�?                int compare_name_with_solution_list;
        ...
        3. 算法参数结构�?
            ...
            typedef struct auxiliary_arguments {
                ...
                double factor;          ///< 计算公式最大公约数�?000 / note�?                size_t counter;         ///< 记录方案数量
    ...
    2. 寻找方案前的一些准备：
        ...
        2. 获取 $perfect\_score$ �?$good\_score$�?
            ```c
    ...
    3. 寻找第一个符合条件的方案组合�?
        1. 基本思路：使用三层循环遍�?$perfect$�?good$�?max\_combo$，找出第一个满足式(9)与条�?10)的参数组合�?
            - 由于 $perfect$ 数和 $max\_combo$ 数较大的情况更符合直觉，所�?$perfect$ �?$max\_combo$ 以其最大值作为起点，$good$ 以最小值作为起点；

        2. 第一层循环：寻找 $perfect$�?
            - 若当�?$perfect$ 值对应的最大分数（即除 $perfect$ 以外都是 $good$，对应代码中�?$args \to score$）已经小�?$goal$，说明往后所有方案组合都不可能达�?$goal$，没有必要再找下去，标志着第一个方案已经找完�?
            若：
        ...
        3. 第二层循环：寻找 $good$，此时需要注意两点：
            ...
            1. 若当�?$perfect$ �?$good$ 值对应的最大分数（即所�?$perfect$ �?$good$ 判定一次性全部连击无断连）已经小�?$goal$，说明对于当前的 $perfect$ �?$good$ 值，往后所�?$max\_combo$ 值都不可能达�?$goal$，此时需要换下一�?$good$ 值；

            2. 若当�?$perfect$ �?$good$ 值对应的最小分数（�?$max\_combo = 1$）已经大�?$goal$，说明对于当前的 $perfect$ 值，往后所有方案组合都不可能达�?$goal$，没有必要再找下去，此时需要换下一�?perfect$值；
    ...
    4. 寻找其它满足条件的方案组�?
        根据�?(9)$，在 $note$ �?$goal$ 的值不变的情况下，我们可得�?$perfect$�?good$�?max\_combo$ 之间有如下变化关系：

        $perfect$ 值不变时，若 $good$ 的�?*增大**（或减小�?20$，对�?$max\_combo$ 的�?*减小**（或增大�?117$。由于在寻找第一个方案组合时�?good$ 以最小值作为起点，故这�?$good$�?值应增大，即�?
        $$
        \begin{cases}
            good \gets good + 20\\
        ...
        �?(11)$中，$max\_combo$ 必然�?*递减至不大于** $117$ 的值，此时说明本次 $perfect$ 值对应方案组合已经找完，需要换下一�?$perfect$ 值进行寻找，即：
        ...
        将式$(12)(13)$代入�?(9)$，设回调 $good$ 后的新的 $max\_combo$ 起始值为 $new\_max\_combo$，得�?
        $$
        \begin{aligned}
            goal
        ...
        在式$(16)$中，根据�?(11)$，有 $good - last\_good = 20n, n \in N$，则 $\frac {good - last\_good}{20} \ge 0$，得 $max\_combo$ 在每�?$perfect$ 周期查找完毕后都会比上一周期多出一�?*基础�?* $9$，�?$perfect$ �?*递减**的，$good$ 的值在每次周期结束后都�?*重置为最初的�?*，即 $last\_good$ ，也就是说，$max\_combo$ 的�?*一定会在某一周期结束后超�?* $perfect + good$ 的值，这与条件$(10)$矛盾。此时的 $last\_good$ 值已经不再有符合条件的方案组合，需要更新，同时也需要更�?$good$ �?$max\_combo$ 的值，即：
        ...
        相对�?$max\_combo$ 每次递减�?$117$ 来说，基础�?$9$ 很小，做完上述操作后�?max\_combo$ **有小�?$0$ 的风�?*，此时需重复�?(12)$ ~ �?(16)$，直�?$max\_combo$ 恢复至大�?$0$�?
    5. 将符合条件的方案组合加入至方案链表：
        ...
        1. $max\_combo$ 超界�?        2. $perfect$ 自减下溢，这种情况标志着**所有方案都已找�?*，没有必要再找下去；
        3. 达成该方案所需最�?note$数超出上界；
        ...
        tail = add_solution_list(tail, args); ///< 返回最后一个方案的地址，方便下次添�?        ```

    6. 查找结束�?
        �?$perfect < 0$ 外，还有一种情况标志着方案已经找完�?last\_good > note$。该情况说明达成 $goal$ 所需最�?$good$ 数已�?*超过**歌曲�?$note$ 总数，此�?以后的所有方案所需 $note$ 数都多于�?$note$ 数，没有继续查找的必要�?
4. #### 结论
...
3. ### 算法设计
    ...
    1. 数据结构设计�?
        ...
        3. 算法参数结构�?
            ...
            typedef struct auxiliary_arguments {
                ...
                double factor;          ///< 计算公式最大公约数�?000 / note�?
    ...
    5. 将符合条件的方案组合加入至方案链表：
        ...
        1. $max\_combo$ 超界�?
        2. $perfect$ 自减下溢，这种情况标志着**所有方案都已找�?*，没有必要再找下去；
        3. 达成该方案所需最�?note$数超出上界；
