...
bool set_data(int* data, Input input, Song* song) {
    ...
    while (status == FAILURE) {
        ...
        ///< 获取用户输入并判断合法�?        status = scanf_int(data, min, input.max);
        if (is_valid(&status, input, song->note)) {
            return true;
        }

        ///< 直接退出确�?        if (errno != EINVAL) {
            continue;
        }
...
Status scanf_int(int* data, int min, int max) {
    ...
    ///< 检查输入情�?    if (!fgets(str, INT_SIZE, stdin)) {
    ...
    ///< 将字符串转换为整�?    char* endptr = NULL;
    long temp = strtol(str, &endptr, 0);

    ///< 检查合法�?    bool invalid = ((*endptr != '\n') && (*endptr != '\0'));
