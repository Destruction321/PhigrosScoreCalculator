...
FILE* create_file(Song* song, Status* status) {
    /**
     * file_path: 文件完整路径
     * folder_path: 文件所在的文件夹路�?     * check_folder_path: 检测指针，若为 NULL，说明用户选择不创建文�?     */
    ...
    ///< 写文�?    FILE* fp = fopen(file_path, "w");
    while (fp == NULL) {
        ///< 访问失败，但失败原因不是文件不存�?        if (errno != ENOENT) {
            break;
        }
        ///< 没有对应目录，尝试创建文件夹
...
char* get_folder_path(char* file_path, char* folder_path, Song* song) {
    ///< 获取solutions文件夹路�?    getcwd(folder_path, FILE_LENGTH);
...
char* set_file_path(char* file_path, Song* song) {
    ...
    ///< 用户选择直接存入默认文件�?    if (!set_folder_path(subfolder)) {
        free(subfolder);
        return NULL;
    }
...
bool set_folder_path(char* subfolder) {
    bool too_long = true;
    while (too_long) {
        printf("（不超过 %d 个半角字�?默认default文件夹）�?, FOLDER_NAME_LAST);
        if (!fgets(subfolder, SUBFOLDER_NAME_LENGTH, stdin)) {
            free(subfolder);
            alloc_error("获取输入失败");
        }

        ///< 用户选择不创建文�?        if (strncmp(subfolder, "0", 1) == 0) {
            return false;
        }

        ///< 用户选择默认文件�?        if (strncmp(subfolder, "\n", 1) == 0) {
...
bool check_file_exist(char* file_path, char* subfolder, Song* song) {
    if (access(file_path, F_OK) != 0) {
        ...
        ///< 文件存在，询问用户是否替�?        int choice = replace_or_skip(subfolder, song);
...
FILE* create_file(Song* song, Status* status) {
    /**
     * file_path: 文件完整路径
     * folder_path: 文件所在的文件夹路�?
     * check_folder_path: 检测指针，若为 NULL，说明用户选择不创建文�?
     */
    char file_path[FILE_LENGTH] = { '\0' };
    char folder_path[FILE_LENGTH] = { '\0' };
    char* check_folder_path = get_folder_path(file_path, folder_path, song);
