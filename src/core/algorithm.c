...
static const double ACCURACY_SCORE = 900000.0; ///< 判定�?static const double COMMON_FACTOR = 5000.0;

double factor = 0; ///< 公式系数�?000 / note�?
...
bool seek_solution(Song* song) {
    ...
    case ALL_GOOD:
        ...
        break;
    default:
        return algorithm(song); ///< 不是特殊分数，进入计算流�?    }
    return true;
}
...
bool algorithm(Song* song) {
    ...
    ///< 过滤不合理数�?    if (args.delta_score == 0.0) {
        return true;
    }
...
bool get_solutions(Song* song, Args args) {
    const int SIZE = 1000; ///< 动态数字初始大�?    array = INITIALIZE_ARRAY(SIZE, s_list);
    fputs("计算中……\n", stdout);
    Loop loop = { 0 };
...
bool get_first_perfect(Song* song, Args args, Loop* loop) {
    do {
        ...
        /**
         * 假设 FULL COMBO
         * �?good = note - perfect，max_combo = note
         */
...
bool get_first_good(Song* song, Loop* loop) {
    do {
        /**
         * 假设所�?perfect �?good 全部连击，中间未断连
         * �?max_combo = perfect + good
         */
        int score = calculate_score(
            loop->loop_perfect,
            loop->loop_good,
...
static const double ACCURACY_SCORE = 900000.0; ///< 判定�?
