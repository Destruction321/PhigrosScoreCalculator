 ...
 * @file main.c
 * 
 * @brief Phigros算分器，助力你打�?14514�? * 
 * 关于算法的说明，请参阅README.md
 * 本程序纯属娱乐，闲得没事就改�? * 祝你在Phigros中取得好成绩（雾
 * 
 * @author 棍母
 * @version 11.45.14
 * @date 1919-08-10
 ...
 */
static void program(Song* song) {
    bool code = true; ///< 退出确认，true 表示未询问，false 表示已询�?    while (true) {
        if (code) {
            fputs("Phigros算分器，助力你打�?14514分\n", stdout);
        }
...
/**
 * @brief 错误退出处理函�? * 
 * @param[in] error 错误信息格式字符�? * @param[in] ... 可变参数列表
 */
static void error_exit(const char* error, ...) {
    ...
    vprintf(error, args);
    va_end(args);
    longjmp(env, 1); ///< 跳转�?main 函数进行错误处理
}
/**
 * @file main.c
 * 
